...
var app=angular.module('movies',['googlechart'])
app.controller("maincontrollermovies", function($scope,$http){
         ...
         $scope.init_trend = function() {
            ...
            $http(req).then(function(data) {
                // console.log(data);
								...
								for(i=0;i<data.data.length;i++){
									...
									myobjlist.trend_title=obj.movie.title
									myobjlist.trend_year=obj.movie.year
									
									myobjgraf.trend_title=obj.movie.title
							 ...
							 $scope.trend_chartObject = {
							  ...
							  "options": {
							    "title": "TRENDING MOVIES INFOGRAPH",
							    "isStacked": "true",
							    "legend" : {position: 'left', alignment :'left', textStyle: {color: 'blue', fontSize: 14,bold:true}},
							    "fill": 40,
        	 ...
        	 $scope.init_anticipated = function() {
			            var req =	{
			            				method: 'GET',
						 				url: 'https://api-v2launch.trakt.tv/movies/anticipated',
						 				headers: {
						   							'Content-Type': 'application/json',
						   							'trakt-api-version':2,
						   							'trakt-api-key':$scope.apiKey
            ...
            $http(req).then(function(data) {
                // console.log(data);
								...
								for(i=0;i<data.data.length;i++){
									...
									myobjlist.anticipated_title=obj.movie.title
									myobjlist.anticipated_year=obj.movie.year
									 
									myobjgraf.anticipated_title=obj.movie.title
        	...
        	$scope.init_popular = function() {
			            var req =	{
			            				method: 'GET',
						 				url: 'https://api-v2launch.trakt.tv/movies/popular',
						 				headers: {
						   							'Content-Type': 'application/json',
						   							'trakt-api-version':2,
						   							'trakt-api-key':$scope.apiKey
 		...
 		$scope.init_collect = function() {
			            var req =	{
			            				method: 'GET',
						 				url: 'https://api-v2launch.trakt.tv/movies/collected/weekly',
						 				headers: {
						   							'Content-Type': 'application/json',
						   							'trakt-api-version':2,
						   							'trakt-api-key':$scope.apiKey
            ...
            $http(req).then(function(data) {
								...
								for(i=0;i<data.data.length;i++){
									...
									myobjlist.collect_title=obj.movie.title
									myobjlist.collect_year=obj.movie.year
									 
									myobjgraf.collect_title=obj.movie.title
							 ...
							 $scope.collect_chartObject = {
							  ...
							  "data": {
							    "cols": [
							        { label: "MOVIE", type: "string"},
							        { label: "NUMBER OF SUBSCRIBERS WITH MOVIE IN THEIR COLLECTIONS", type: "number"}
							    ], "rows":  [
							  ...
							  "options": {
							    "title": "MOVIES INCLUDED MOSTLY IN COLLECTIONS",
							    "isStacked": "true",
							    "fill": 20,
							    "displayExactValues": true,
							    "backgroundColor": { fill:'transparent' },
        	...
        	$scope.init_boxoffice = function() {
			            var req =	{
			            				method: 'GET',
						 				url: 'https://api-v2launch.trakt.tv/movies/boxoffice',
						 				headers: {
						   							'Content-Type': 'application/json',
						   							'trakt-api-version':2,
						   							'trakt-api-key':$scope.apiKey
            ...
            $http(req).then(function(data) {
                // console.log(data);
								...
								for(i=0;i<data.data.length;i++){
									...
									myobjlist.boxoffice_title=obj.movie.title
									myobjlist.boxoffice_year=obj.movie.year
									 
									myobjgraf.boxoffice_title=obj.movie.title
									myobjgraf.boxoffice_revenue=obj.revenue
							 ...
							 $scope.boxoffice_chartObject = {
							  ...
							  "data": {
							    "cols": [
							        { label: "MOVIE", type: "string"},
							        { label: "REVENUE GENERATED IN $", type: "number"}
							    ], "rows":  [
							  ...
							  "options": {
							    ...
							    "hAxis": {
							      "title": "BOX OFFICE REVENUE (WEEKLY UPDATED)"
							    },chartArea:{left:80,top:50,width:"100%",height:"80%"},
      							colors: ['teal','green'],
							  },
							  "formatters": {}
